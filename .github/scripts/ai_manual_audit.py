...
def get_fork_diff(owner, repo, target_fork, token):
    """Receiving diff between own master and master of target (foreign) fork."""
    if not target_fork:
        return ""
...
def main():
    ...
    - 🚨 Критические уязвимости и баги (если есть), как их исправить (с примером кода)
...
# Protect from error 429: Retry mechanism
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            # Got success answer, exit from loop
            issue_title = f"🤖 AI Audit Report: Internal analys"
            if target_fork:
                issue_title += f" & Compare with {target_fork}"
            ...
            if "429" in error_str or "quota" in error_str.lower():
                wait_time = 15 # Wait 15 seconds before every retry
                print(f"Attempt {attempt + 1}/{max_retries} fails because of limits (429). Waiting {wait_time} seconds...")
                time.sleep(wait_time)
            else:
                print(f"Critical error when API calling: {error_str}")
                break
