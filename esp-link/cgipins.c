...
int ICACHE_FLASH_ATTR cgiPinsGet(HttpdConnData *connData) {
  ...
  len = os_sprintf(buff,
      "{ \"reset\":%d, \"isp\":%d, \"conn\":%d, \"ser\":%d, \"swap\":%d, \"rxpup\":%d, \"pinvert\":%d, \"txen\":%d }",
      ...
      flashConfig.pin_invert, flashConfig.tx_enable_pin);
...
int ICACHE_FLASH_ATTR cgiPinsSet(HttpdConnData *connData) {
  ...
  if (ok > 0) {
    ...
    os_printf("Pins changed: reset=%d isp=%d conn=%d ser=%d swap=%d rx-pup=%d pinvert=%X\n tx_enable=%d\n",
	  reset, isp, conn, ser, swap, rxpup, pinvert, tx_enable);
